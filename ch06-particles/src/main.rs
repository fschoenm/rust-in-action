...
impl Particle {
    fn new(world: &World) -> Particle {
        ...
        let y = world.height;
        let x_velocity = 0.0;
        let y_velocity = rng.gen_range(-2.0..0.5);
...
impl World {
    ...
    fn add_shapes(&mut self, count: usize) {
        for _ in 0..count {
            self.particles.push(Box::new(Particle::new(&self)));
        }
    }

    fn remove_shapes(&mut self, count: usize) {
        for _ in 0..count {
